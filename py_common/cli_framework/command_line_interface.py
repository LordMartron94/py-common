...
class CommandLineInterface:
	...
	def __init__(self,
	             ...
	             log_module_sep: str = "CLI",
				 use_categories: bool = True) -> None:
		"""
        ...
        :param use_categories: Whether to use categories (default is True).
		...
		self._use_categories: bool = use_categories

		if initialize_with_help_command:
			self.add_command(["help", "?"], "Displays this help message.", self.print_help, category="DEFAULT")

		self.add_command(['cheese'], "Easter egg.", self._cheese_command, category="DEFAULT")
		self.add_command(['tea'], "Easter egg.", self._tea_command, category="DEFAULT")
		self.set_exit_command(exit_command)
		self._separator = log_module_sep

		self._logger.trace("Successfully initialized CommandLineInterface", separator=self._separator)
	...
	def set_exit_command(self, exit_command: Callable):
		...
		if not self._command_exists(keys):
			self.add_command(keys, "Quits the application.", exit_command, category="DEFAULT")
	...
	def add_command(self, keys: List[str], description: str, action: Callable, arguments: List[Any] = None, category: str = "Software") -> None:
		"""
		...
		command_model = CommandModel(
			commands=[f"{self._command_prefix}{key}" for key in keys],
	...
	def update_command(self, keys: List[str], description: str, action: Callable, arguments: List[Any], category: str) -> None:
		"""
	...
	@staticmethod
	def _group_commands(commands: List[CommandModel]) -> Dict[str, List[CommandModel]]:
		grouped: Dict[str, List[CommandModel]] = defaultdict(list)
	...
	def print_help(self, print_to_logger_also: bool = False) -> None:
        ...
        """
		grouped_commands: Dict[str, List[CommandModel]] = self._group_commands(self._commands)

		for category, commands in grouped_commands.items():
			if self._use_categories:
				print(self._get_formatted_category(category))

			commands.sort(key=lambda x: x.commands[0])  # Sort by command key
	...
	def _get_formatted_category(self, category: str):
		return self._color_helper.colorize_string(f"--- {category} ---", text_color_hex="#808080")
